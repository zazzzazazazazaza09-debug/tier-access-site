...
alter table profiles add column if not exists signup_ip_hash text;
alter table profiles add column if not exists signup_device_hash text;
...
create index if not exists reward_clicks_created_at_idx on reward_clicks(created_at desc);
