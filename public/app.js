...
async function loadMe() {
  ...
  if (nextTier) {
    badge.textContent = `🎯 ${nextTier.invitesRequired - count} more invites to unlock ${nextTier.name}`;
  } else {
  ...
  if (starterTiers.length) {
    ...
    for (const t of starterTiers) {
      ...
      const inviteBtn = el("button", {
        ...
        ? `✅ Open ${t.name}`
        : left > 0 ? `🔗 ${left} more invite${left > 1 ? "s" : ""} to unlock` : "Claim free access"
      );
...
function cryptoFullName(t) {
  return ({ BTC: "Bitcoin", ETH: "Ethereum", LTC: "Litecoin", SOL: "Solana" })[t] || t;
}

function updateCryptoTxCount() {
  $("cryptoTxCount").textContent = `• ${$("cryptoTxId").value.trim().length} characters`;
}
...
async function submitCrypto() {
  ...
  await submitPurchase(purchasePayload("crypto", { crypto_currency: activeCrypto, crypto_amount: amount, tx_id: txId }));
}
...
function setModalMsg(text, error = false) {
  const m = $("modalMsg"); m.textContent = text;
...
function renderCustomCategories() {
  const wrap = $("customCategories"), cp = CFG.customPack;
  if (!wrap || !cp) return;
  wrap.innerHTML = "";
...
function renderCustomSizes() {
  const wrap = $("customSizes"), cp = CFG.customPack;
  if (!wrap || !cp) return;
  wrap.innerHTML = "";
...
async function loadMyCustomOrders() {
  ...
  try {
    ...
    for (const o of orders) {
      const btn = el("button", { class: "custom-order-item", type: "button", onclick: () => openChatModal(o.id) }, [
...
function renderChatFloat() {
  ...
  for (const o of openOrders) {
    ...
    const btn = el("button", { class: "chat-float-btn" + (hasPrice ? " has-price" : ""), type: "button", onclick: () => openChatModal(o.id) }, [
...
function generateFakeNotifications(count) {
  const types = ["purchase","invite","custom"], list = [];
  for (let i = 0; i < count; i++) {
    ...
    else if (type === "invite") text = `<b>${name}</b> invited ${Math.floor(Math.random() * 8) + 3} friends today`;
    else text = `<b>${name}</b> started a custom order`;
...
function initNotifications() {
  ...
  for (const n of notifs) {
    list.appendChild(el("div", { class: "notif-item" }, [
      ...
      el("div", { class: "notif-body" }, [el("div", { html: n.text }), el("small", { class: "muted" }, `${n.mins} min ago`)])
    ]));
  }
...
function initBackground() {
  ...
  function resize() {
    ...
    stars = Array.from({ length: count }, () => ({ x: Math.random() * width, y: Math.random() * height, r: Math.random() * 1.6 + .35, d: Math.random() * .65 + .08 }));
  }
  function draw() {
