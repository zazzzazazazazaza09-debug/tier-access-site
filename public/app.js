...
async function openTier(tier) {
  try {
    await request("claim", { method: "POST", body: JSON.stringify({ tier_id: tier.id }) });
    ...
    openTierContent(tier.id, tier.name);
  } catch (err) { alert(err.message); }
}

/* ================================================================
   TIER CONTENT MODAL — embedded video player + ZIP download
================================================================ */
let tierContentVideos = [];
...
function getVideoEmbed(url) {
  ...
  if (/dropbox\.com/.test(url)) {
    const direct = url.replace("www.dropbox.com", "dl.dropboxusercontent.com").replace(/[?&]dl=\d/, "").replace(/\?$/, "");
    return { type: "video", src: direct + (direct.includes("?") ? "&" : "?") + "raw=1" };
  }
...
function renderVideoCard(v, index) {
  ...
  const playerHtml = embed.type === "video"
    ? `<video src="${v.video_url}" controls preload="metadata" class="video-player"></video>`
    : `<iframe src="${embed.src}" class="video-player" allowfullscreen allow="autoplay; encrypted-media" frameborder="0"></iframe>`;

  const card = el("div", { class: "video-card" });
  const embedWrap = el("div", { class: "video-embed-wrap" });
  embedWrap.innerHTML = playerHtml;
  card.appendChild(embedWrap);

  const info = el("div", { class: "video-card-info" });
  if (v.title) info.appendChild(el("div", { class: "video-card-title" }, v.title));

  const dlBtn = el("a", {
    class: "ghost-mini video-dl-btn",
    href: v.video_url,
    target: "_blank",
...
async function openTierContent(tierId, tierName) {
  const modal = $("tierContentModal");
  const titleEl = $("tierContentTitle");
  const subEl = $("tierContentSub");
  const videosWrap = $("tierContentVideos");
  const msgEl = $("tierContentMsg");

  titleEl.textContent = tierName || (tierId === -1 ? "Preview" : `Tier ${tierId}`);
  subEl.textContent = tierId === -1 ? "Free preview content" : "Your exclusive content";
  videosWrap.innerHTML = "<p class='muted' style='text-align:center;padding:24px'>Loading…</p>";
  msgEl.textContent = "";
  modal.classList.remove("hidden");
  tierContentVideos = [];

  try {
    const data = await request(`tier-videos?tier_id=${tierId}`);
    tierContentVideos = data.videos || [];
    videosWrap.innerHTML = "";
    if (!tierContentVideos.length) {
      videosWrap.innerHTML = "<p class='muted' style='text-align:center;padding:32px'>No content available yet — check back soon.</p>";
    } else {
      for (let i = 0; i < tierContentVideos.length; i++) {
        videosWrap.appendChild(renderVideoCard(tierContentVideos[i], i));
      }
    }
  } catch (err) {
    videosWrap.innerHTML = "";
...
function closeTierContent() { $("tierContentModal").classList.add("hidden"); tierContentVideos = []; }

async function downloadAllAsZip() {
  if (!tierContentVideos.length) return;

  const btn = $("downloadAllZipBtn");
  ...
  try {
    ...
    const titleEl = $("tierContentTitle");
    const folderName = (titleEl ? titleEl.textContent : "content").replace(/[^a-z0-9]/gi, "_");
    const folder = zip.folder(folderName);

    const lines = tierContentVideos.map((v, i) => `${i + 1}. ${v.title || "Video " + (i + 1)}\n${v.video_url}`);
    folder.file("links.txt", lines.join("\n\n"));

    const indexHtml = `<!DOCTYPE html><html><head><title>${folderName} Videos</title>
<style>body{font-family:sans-serif;padding:20px;background:#111;color:#eee;max-width:900px;margin:0 auto}
h1{background:linear-gradient(90deg,#00f5ff,#7b2ff7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.v{margin:28px 0;padding:18px;border:1px solid #333;border-radius:10px}
.v h2{margin:0 0 12px;font-size:1rem}
a{color:#00f5ff}</style></head><body>
<h1>${folderName} Videos</h1>
${tierContentVideos.map((v, i) => `<div class="v"><h2>${i + 1}. ${v.title || "Video " + (i + 1)}</h2>
<p><a href="${v.video_url}" target="_blank">${v.video_url}</a></p></div>`).join("")}
</body></html>`;
    folder.file("index.html", indexHtml);

    for (let i = 0; i < tierContentVideos.length; i++) {
      const v = tierContentVideos[i];
      const embed = getVideoEmbed(v.video_url);
      if (embed.type === "video") {
        try {
          ...
          if (resp.ok) {
            ...
            const ext = v.video_url.split("?")[0].split(".").pop() || "mp4";
            const fname = `${String(i + 1).padStart(2, "0")}_${(v.title || "video").replace(/[^a-z0-9]/gi, "_")}.${ext}`;
            folder.file(fname, blob);
          }
        } catch (_) { /* CORS or network error — URL already in links.txt */ }
      }
    }
...
function handleDrawerAction(action) {
  ...
  switch (action) {
    ...
    case "preview": openTierContent(-1, "Preview"); break;
...
function bindStaticUI() {
  ...
  $("tierContentClose").addEventListener("click", closeTierContent);
